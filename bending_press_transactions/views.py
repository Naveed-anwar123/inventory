...
from bending_press_items.models import Item
...
def add_transaction(request):
    ...
    return render(request , 'btransactions/add_transaction.html',{'items':item_list})
...
def insert_transaction_to_db(request):
    if request.method == 'POST':
        ...
        lc = json.loads(request.POST.get('lc'))
        ...
        for i,i_,p,q,t,l in zip(item_ids,inames,prices,quantities,totals,lc):
...
def view_transactions(request):
    #transactions = IronStoreTransactionHistory.objects.select_related('receipt').filter(damount__gte=-1).order_by('-added_at').distinct().all()
    ...
    p2.total_amount\
    FROM\
        bending_press_transactions_ironstoretransactionhistory p1\
    INNER JOIN bending_press_transactions_ironstoretransaction p2 ON\
    ...
    WHERE\
        ...
        SELECT\
            MAX(added_at)\
        FROM\
            bending_press_transactions_ironstoretransactionhistory\
        WHERE\
    ...
    for t in transactions:
        ...
        print(t.sold_quantity)
    """
    return render(request , 'btransactions/view_transactions.html',{'transactions':transactions})
...
def test(request):
    ...
    return render(request,'btransactions/test.html' ,{'transactions':transactions} )
...
class Pdf(View):
    ...
    def get(self, request,receipt):
        ...
        for t in transactions:
            ...
            total = float(total) + float(t.item_total_price)
        ...
        return Render.render('btransactions/test.html', {'transactions':transactions, 'receipt_id':receipt_id , 'total':total,'discount':discount,'damount':dm,'paid':pd,'cname':cname,'phone':phone,'date':date,'udhar':trans.advance})
...
class Pdf_(View):
    ...
    def get(self, request,receipt):
        ...
        for t in transactions:
            ...
            phone = t.receipt.phone    
            date = "{}".format(t.added_at.strftime("%B %d, %Y %H:%M:%S"))
        return Render.render('btransactions/test.html', {'transactions':transactions, 'receipt_id':receipt_id , 'total':total,'discount':discount,'damount':trans.damount,'paid':trans.paid,'cname':cname,'phone':phone,'date':date})
...
def daily_sale(request):
    ...
    *\
    ...
    WHERE\
        p1.today_date = "{date}"\
    ')
    return render(request,'btransactions/daily_sale.html',{'transactions':transactions})        
...
class Pdf_sale_report(View):
    def get(self,request, date):
        ...
        *\
        ...
        print(expenses)
        return Render.render('btransactions/daily_sale_report.html',
